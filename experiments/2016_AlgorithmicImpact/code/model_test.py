...
class Test(object):
    ...
    def __init__(self, start, training_d, gap, testing_d):
        ...
        if self.start.year == 2014:
            ...
            self.training_table = "tweet_as_json_15"
        else:
    ...
    # fetch tweets
        ...
        for tw in tweets_raw:
            if (excluded is None or (tw.user_screen_name not in excluded
                ...
                tweets.append(tw)
        if phase == "training":
            tweets = self.filter_geometry(tweets, args.ses, cur, args.how_filter, args.train_tweet_limit)
            users = set()
            for tw in tweets:
               users.add(tw.user_screen_name)
    ...
    def filter_geometry(self, tweets, ses, cur, how, limit):
        ...
        if ses == 'urban':  # By county: 1 = most urban, 6 = most rural
            weights = {'balanced' : {1:0.305, 2:0.248, 3:0.206, 4:0.092, 5:0.088, 6:0.062},
                       'expected' : {1:0.396, 2:0.237, 3:0.195, 4:0.081, 5:0.063, 6:0.028},
                         'urbanf' : {1:0.500, 2:0.500, 3:0.000, 4:0.000, 5:0.000, 6:0.000},
                         'ruralf' : {1:0.000, 2:0.000, 3:0.000, 4:0.000, 5:0.500, 6:0.500},
                         'urban1' : {1:0.435, 2:0.261, 3:0.215, 4:0.089, 5:0.000, 6:0.000},
                         'rural1' : {1:0.244, 2:0.198, 3:0.165, 4:0.074, 5:0.209, 6:0.111},
                         'rural2' : {1:0.183, 2:0.149, 3:0.124, 4:0.055, 5:0.330, 6:0.160},
                         'rural3' : {1:0.122, 2:0.099, 3:0.082, 4:0.037, 5:0.450, 6:0.210},
                         'rural4' : {1:0.061, 2:0.050, 3:0.041, 4:0.018, 5:0.571, 6:0.259},
                         'rural5' : {1:0.000, 2:0.000, 3:0.000, 4:0.000, 5:0.692, 6:0.308},
                      'surburban' : {1:0.000, 2:0.000, 3:0.500, 4:0.500, 5:0.000, 6:0.000},
                        'extra_r' : {1:0.396, 2:0.237, 3:0.195, 4:0.081, 5:0.163, 6:0.128},
                        'extra_a' : {1:0.396, 2:0.396, 3:0.396, 4:0.396, 5:0.396, 6:0.396}}
            for w in weights:
                sum = 0
            ...
            weights = {'balanced'    : {'m':0.5, 'f':0.5},
                       'male-only'   : {'m':1.0},
                       'female-only' : {'f':1.0}}
        else:  # for county-level attributes that aren't binned
            weights = {how:{}}
        ...
        if ses == 'pop_pct' or ses == 'area_pct':
            ...
            counties = {}
            for county in cur:
                if ses == 'pct15to34':  # assign counties their respective age bin
                    if county[1] < 20:
                ...
                else:  # copy over bin from PostgreSQL query
        ...
        if wins:
            self.map_tweets(wins, "{0}/test_tweets_{1}.csv".format(outputdir, self.i),
                            ...
                            properties=['ntokens', 'ncomponents', 'npoints',
                                        ...
                                        'contour', 'pra50', 'pra90', 'pra95',
                                        'covt95', 'covt90', 'covt50'])
        # Generally all tweets are successfully given an estimate by the models, but on occasion there are a few that can't be estimated
        if losses:
            try:
                ...
                print("Failed to map out {0} losses.".format(len(losses)))
